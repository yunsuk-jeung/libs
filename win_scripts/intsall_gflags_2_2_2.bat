...
@REM REM Run CMake for the project with a custom install prefix
@REM %CMAKE_PATH% -G %VS_VERSION% -DCMAKE_INSTALL_PREFIX=%INSTALL_DIR% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% ^
@REM -D BUILD_SHARED_LIBS=OFF ^
@REM -D BUILD_STATIC_LIBS=ON ^
@REM -D BUILD_gflags_LIB=ON ^
@REM -D BUILD_TESTING=OFF ^
@REM -D INSTALL_SHARED_LIBS=OFF ^
@REM -D INSTALL_STATIC_LIBS=OFF ^
@REM ..